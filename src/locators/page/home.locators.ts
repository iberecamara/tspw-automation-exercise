...
export class HomeLocators {
  ...
  constructor(page: Page) {
    ...
    this.recommendedItemsProducts = this.recommendedItemsContainer.locator(
      ".item.active .productinfo",
    );
    this.recommendedProductsId = (locator: Locator) => {
      return locator.locator("a");
    };
    this.recommendedProductsName = (locator: Locator) => {
      return locator.locator("p");
    };
    this.recommendedProductsPrice = (locator: Locator) => {
      return locator.locator("h2");
    };
    this.addRecommendedItem = (id: number) => {
      return page
        .locator(`.item.active a[data-product-id="${id}"]`)
        .filter({ visible: true });
    };
    this.subheading = page.getByRole("heading", {
