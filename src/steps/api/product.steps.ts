...
export class ProductApiSteps {
    ...
    // Actions
    async getAllProducts(options?: { raw?: boolean, method?: 'POST' | 'GET', brand?: string }): Promise<CustomResponseType | ProductType[]> {
        if (options?.raw) {
    ...
    // Validations
    async validateGetAllProducts(response: CustomResponseType): Promise<void> {
