...
export class UserApiSteps extends BaseSteps {
    ...
    async deleteAccount(user: UserType): Promise<CustomResponseType> {
        ...
        await test.step('Delete user via API', async () => {
            response = await this.userApi.deleteUser(user.email, user.password ?? EMPTY);
        });
    ...
    async validateUpdateUserResponse(response: CustomResponseType): Promise<void> {
    ...
    async validateUpdatedUser(updatedUser: UserType, retrievedUser: UserResponseType): Promise<void> {
