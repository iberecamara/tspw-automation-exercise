...
export class AccountCreatedDeletedSteps {
    ...
    constructor(logger: TestAutomationLogger, accountCreatedDeletedPage: AccountCreatedDeletedPage) {
        this.logger = logger;
    ...
    async clickContinue(page: string): Promise<void> {
        ...
        await test.step(`Click Continue in Account ${page} page`, async () => {
            ...
            await this.accountCreatedDeletedPage.page.waitForLoadState('load', { timeout: 15 * MINUTE_IN_MILISSECONDS });
        });
    ...
    async validateAccountActionText(action: string): Promise<void> {
        this.logger.debug(`Validating that 'Account ${StringUtils.capitalize(action)}!' text is displayed`);
        const locator: Locator = action === CREATED ? this.accountCreatedDeletedPage.locators.accountCreatedText : this.accountCreatedDeletedPage.locators.accountDeletedText
