  ...
  () => {
    ...
    test(
      ...
      async ({ userApiSteps }) => {
        ...
        const response: CustomResponseType =
          await userApiSteps.updateAccount(updatedUser);
        await userApiSteps.validateUpdateUserResponse(response);
        const retrievedUserResponse: CustomResponseType =
          await userApiSteps.getUserByEmail(updatedUser.email);
        const { user: retrievedUser = {} as UserResponseType } =
          retrievedUserResponse.body;
