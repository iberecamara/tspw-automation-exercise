...
export class DateTimeUtils {
    ...
    public static getDateTime(options?: { fileFormat?: boolean, onlyNumbers?: boolean }): { date: string, time: string, datetime: string } {
        ...
        const datetime = options?.onlyNumbers ? `${date.replaceAll('-', EMPTY)}${time.replaceAll(':', EMPTY)}` : `${date}${options?.fileFormat ? `_${time.replaceAll(':', '-')}` : ` ${time}`}`;
