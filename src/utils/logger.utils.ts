...
export class TestAutomationLogger {
    ...
    public static async splitGeneratedLogs(): Promise<void> {
        ...
        for (const logFile of logFiles) {
            ...
            for (const executionContent of TestAutomationLogger.splitIntoExecutionBlocks(content)) {
    ...
    private static extractExecutionTag(content: string): string {
        const match = content.match(new RegExp(`@${Environment.JIRA_BOARD}-[A-Za-z0-9._-]+`, 'i')) ?? content.match(new RegExp(`${Environment.JIRA_BOARD}-[A-Za-z0-9._-]+`, 'i'));
    ...
    private static isCompleteExecutionBlock(content: string): boolean {
        ...
        const hasStartMarker = lines.some((line) => TestAutomationLogger.getBoundaryMarker(line) === '*');
        const hasEndMarker = lines.some((line) => TestAutomationLogger.getBoundaryMarker(line) === '#');
        return hasStartMarker && hasEndMarker && TestAutomationLogger.isMeaningfulExecutionBlock(trimmedContent);
    }
    ...
    static async removeTempFiles(): Promise<void> {
        await TestAutomationLogger.removeTempFilesFromDirectory(path.resolve(LOG_FOLDER));
    }
